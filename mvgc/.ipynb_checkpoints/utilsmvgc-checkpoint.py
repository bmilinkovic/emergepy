...
def  spectralnorm(A, rhoa=None):
    ...
    if rhoa is None:
        _, rho = rho_intemediary, "The spectral norm of the VAR coefficients that potentially doesn't satisfy the stability"
        return rho   
    else:
        A = VAR_spectral_radius_decay(A, rhoa/rho_intemediary)
        rho = rho_intemediary
        return A, rho
...
def spectrallimit(A, M, r1, r2):
    """
    Identifies the minnimum and maximum 'r' between which minimum phase (VMA) and stability (VAR)
    hold. The minimum phase condition is that the spectral norm of the VMA coefficients is less than 1.
    ...
    """
    assert spectralnorm(A - r1*M, 2) > 1 and spectralnorm(A - r2*M, 2) < 1, "The spectral norm of the VAR coefficients must be less than 1"

    while True:
        r = (r1 + r2) / 2
        if spectralnorm(A - r*M, 2) < 1:
            r2 = r
        else:
            r1 = r
        if np.abs(r2 - r1) < 1e-6:
            break
    return r
...
def VAR_spectral_radius_decay(A, decay_factor):
    if np.ndim(A) == 1:
        p = len(A)
        p1 = p - 1
        A1 = np.vstack((A.reshape(1, p), np.hstack((np.eye(p1), np.zeros((p1, 1))))))  # VAR coefficients for 1-lag problem
        A1_decay = A1 * decay_factor
        return A1_decay[0, :]
    else:
        n, n1, p = A.shape
        assert n1 == n, "VAR coefficients matrix has bad shape"
        pn1 = (p - 1) * n
        A1 = np.vstack((A.reshape(n, p * n), np.hstack((np.eye(pn1), np.zeros((pn1, n))))))
        A1_decay = A1 * decay_factor
        return A1_decay[:n, :].reshape(n, n, p)
...
def iss_rand(n, m, rhoa, dis=False):
    ...
    if dis is None:
        ...
        A, _ = spectralnorm(np.random.randn(m, m), rhoa)
